...
> **✅ اتصلح (2026-09-01) — والخطورة طلعت أعلى من التقدير الأصلي (🟠 → 🔴).**
>
> البند اتفتح تاني لما تلات تستات في `coachEvaluations.test.js` فشلت من غير أي تغيير
> في كود التطبيق: الساعة عدّت 2026-08-31 21:53Z (= 00:53 محلي على GMT+3)، و
> `isCurrentMonth` — اللي مكانش مذكور في الجدول تحت أصلاً — كان بيقارن **محلي**
> بينما `year`/`month` الجايين من العميل متولّدين UTC. النتيجة مش عرض غلط: **قفل
> الـblind review كان بيتخطّى بالكامل**، فأي أدمن يقدر يقرا تقييم أدمن تاني قبل ما
> ينشر بتاعه — لمدة 3 ساعات عند كل حدود شهر. ده تسريب صلاحيات مقيس، مش نظري.
>
> اتعمل `Backend/utils/time.js` كمصدر واحد (`isCurrentMonthUTC` /
> `currentYearMonthUTC` / `yearOfUTC` / `startOfTodayUTC` / `utcDayRange`)، وكل
> المواضع في الجدول تحت اتحوّلت ليه — زائد موضعين مكانوش مرصودين:
> `coachEvaluationController.isCurrentMonth` و `playedModel.getBirthYear`
> (الأخير كان بيدّي **سنة ميلاد ناقصة سنة** لمواليد أول يناير على أي سيرفر بتوقيت
> سالب → فئة عمرية غلط أو رفض تسجيل).
>
> التحقق: 9 تستات جديدة (4 في `coachEvaluations.test.js` عند حدود الشهر بالظبط +
> 5 في `tests/timeUtc.test.js`)، كلها بتفرض `process.env.TZ` عشان تفشل حتمياً بدل
> ما تستنى الحدود تيجي مرة كل شهر. **كل الأربعة اتأكد إنها بتفشل على الكود القديم**
> قبل الإصلاح. السويت كامل 791/791. القاعدة اتكتبت في `CLAUDE.md`.
