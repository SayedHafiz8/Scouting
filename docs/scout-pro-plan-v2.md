...
5. `user-form.component.ts:170-173` — قائمة اختيار الرول في فورم "إنشاء/تعديل مستخدم" مكتوبة
   بإيد بثلاث خيارات ثابتة (`coach`, `admin`, `observer`)، مش مبنية من `UserRole` المشتق من
   `openapi.json`. النتيجة: الأدمن مايقدرش يسند رول `proScout` لمستخدم من الواجهة إطلاقاً —
   الإسناد ممكن بس عبر استدعاء API مباشر. يستاهل مرحلة صغيرة منفصلة بعد المرحلة 7، أو
   يتضاف كـ task في المرحلة 4 لو اتقرر إنه لازم يتم قبل أي اختبار يدوي كامل للرول الجديد.
