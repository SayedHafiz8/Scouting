...
} from "../utils/coachEvaluationCriteria.js";
...
const statsShape = {
    ...
    playersManaged:  { type: Number, default: 0 }, // إجمالي اللاعبين المسجلين تحته (لقطة وقت الحفظ)
...
const coachEvaluationSchema = new mongoose.Schema(
    {
        ...
        //
        // §12 — مش required عمداً: لما يتمسح الأدمن نهائياً بنصفّر الحقل ده
        // والتقييم يفضل، عشان حذف أدمن واحد مايمسحش تاريخ تقييم كل الكشافين
        // اللي قيّمهم. الإنشاء لسه بيحطّه دايماً سيرفر-سايد.
        evaluator: {
            type: mongoose.Schema.ObjectId,
            ref: "User",
        },
        ...
        // بيتحسب تلقائي = متوسط كل المعايير
        overallRating: {
            type: Number,
            min: 1,
            max: 10,
...
coachEvaluationSchema.pre("save", function () {
...
coachEvaluationSchema.pre("findOneAndUpdate", async function () {
...
//
// §12 — partial على evaluator لنفس سبب ScoutingReport: لما يتمسح أدمن بنصفّر
// evaluator → null والتقييم بيفضل. والقاعدة اللي فوق دي بالذات هي اللي بتخلي
// التصادم ممكن — لو أدمنين قيّموا نفس الكوتش في نفس الشهر واتمسحوا الاتنين،
// الدوكيومنتين بيبقوا {coach, evaluator: null, year, month} متطابقين وبيوقفوا
// الحذف. الاستثناء بيخلي الفرادة على التقييمات اللي ليها مُقيّم معروف بس.
coachEvaluationSchema.index(
