...
npm start                        # nodemon server.js — needs config.env (see config.env.example)
                                 # config.env MUST declare DB_TARGET=local|staging|production;
                                 # startup aborts if it is missing, or if DB_TARGET=production
                                 # while NODE_ENV is not production. Cron jobs stay off unless
                                 # RUN_JOBS=true (deliberately — two of them delete permanently).
...
- **Every date comparison on the server is UTC, and goes through `Backend/utils/time.js` — no exceptions.** Every date the client sends is generated in UTC: `<input type="date">` becomes UTC midnight, and `year`/`month` come off a `<select>` as bare numbers. Comparing those against `getFullYear()`/`getMonth()`/`getDate()`/`setHours()` — all server-**local** — opens a window as wide as the server's UTC offset in which the two sides disagree about what "today" or "this month" is. That is an access-control hole, not a display detail: on 2026-08-31 21:53Z (00:53 local on a GMT+3 box) `isCurrentMonth` in `coachEvaluationController` returned false for month 8, so the blind-review lock was skipped outright and any admin could read another admin's unpublished evaluation for three hours at every month boundary. Use `isCurrentMonthUTC`, `currentYearMonthUTC`, `yearOfUTC`, `startOfTodayUTC`, `utcDayRange`; add to that file rather than writing a comparison inline. Tests must build their fixtures from the same helpers (or `Date.UTC(...)`) — a test that says `new Date(2026, 5, 16)` is asserting the server's local timezone, which is exactly the bug. Local formatting is the frontend's job.
- **Every `mongoose.connect()` — including `config/database.js` — MUST pass `autoIndex: false, autoCreate: false` explicitly. No exceptions, no environment conditions.** Mongoose defaults `autoIndex: true`, so merely opening a connection fires `Model.init()` → `createIndexes()` and builds every index declared in the schema, as a side effect, before your script's first line of real work runs. A read-only report or a data migration must never reshape indexes. This is not theoretical: on 2026-08-26 a `sync-indexes` run *without* `--apply` — a documented dry run — created `Player.createdBy_1_createdAt_-1` and the unique `ScoutingReport.player_1_coach_1_seasonMatch_1` on production. Had duplicates existed, an exploratory dry run would have kicked off a failing (or, on a large collection, long-running) unique-index build.
  - The **only** sanctioned path for changing indexes is `scripts/syncAllIndexes.js`, which diffs with `Model.diffIndexes()` and is dry-run by default. It works fine with `autoIndex: false` because all of its work is explicit.
  - Write the options **inline** at the call site, not via a shared constant — the rule is meant to be greppable (`mongoose.connect(` with `autoIndex` beside it), and a named constant makes a compliant file look non-compliant.
  - If a script genuinely needs indexes built (e.g. `seedLoadTest.js` before `explainQueries.js` measures anything), call `mongoose.connection.syncIndexes()` **explicitly after** the bulk insert. That is both faster than building them up-front and honest about when it happens.
  - **`config/database.js` used to be exempt from this rule, and that exemption caused an incident.** It carried `autoIndex: !isProduction` plus an unconditional `syncIndexes()` for non-production, both justified as "dev convenience". On 2026-09-12 a `nodemon` dev server with `NODE_ENV=development` was pointed at live production Atlas; saving three `*Model.js` files triggered a restart, and the restart applied the full schema→database index diff to production — **5 drops and 7 creates, with no `--apply`, no confirmation, and nobody watching.** `syncIndexes()` drops as well as creates, so this was not recoverable by simply re-running anything. The autosync is now deleted and `autoIndex` is `false` unconditionally.
  - **Consequence for local development:** after pulling a schema change that adds or alters an index, your local database will not have it until you run `npm run sync-indexes -- --apply` once. Queries still work in the meantime, just unindexed. Tests are unaffected — `tests/setup.js` opens its own connection to an in-memory Mongo where building indexes is correct and disposable.

- **`DB_TARGET` is required in `config.env` — `local`, `staging`, or `production`.** It declares which database `CONNECTION_STRING` actually points at. `config/database.js` refuses to connect when `DB_TARGET=production` and `NODE_ENV` is anything else, and refuses to connect at all when the value is missing or misspelled. There is no default: guessing this is what let a dev server write to production. It is declared rather than sniffed from the hostname on purpose — hostname detection would block a legitimate Atlas *dev* cluster and wave through self-hosted production. The deliberate escape hatch is `ALLOW_DEV_AGAINST_PROD=i-understand-this-writes-to-production` (that exact phrase; `true` is rejected because `true` is easy to set by reflex and easy to leave set), and using it prints a boxed warning at startup. `assertDbTarget()` is exported and unit-tested in `tests/startupGuards.test.js`.
- **Background jobs only run when `RUN_JOBS=true`, and the default is OFF.** `server.js` used to schedule all four `node-cron` jobs unconditionally, so *every* running instance scheduled them — including a developer's laptop pointed at production. Two of them delete permanently and irreversibly: `cleanupDeactivated` (03:00) hard-deletes users 30+ days deactivated along with their Bunny CDN assets, and `mediaRetention` (03:30) purges cold media and orphaned videos along with their Bunny bytes. The default is OFF because the failure directions are not symmetrical: a forgotten flag in production means data accumulates, which one run fixes; the reverse deletes, which only a backup fixes. Both branches log explicitly at startup so a missing flag in production is visible on the first boot rather than discovered weeks later. **Exactly one deployed instance should carry the flag** — `RUN_JOBS` prevents the laptop case but does *not* prevent double execution if production ever runs more than one instance; that needs a database-level lock and is not built yet. The gate lives in `socket/handlers/index.js` (`startBackgroundJobs`) rather than inline in `server.js` so it can be tested.
