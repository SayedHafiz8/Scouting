...
describe('SidebarComponent — role-based navigation menu', () => {
  ...
  it('proScout sees exactly: Dashboard, Players, My Matches, Profile (Stage 6 — DF-002 discharged)', async () => {
    await setup('proScout');
    expect(menuHrefs()).toEqual(['/dashboard', '/players', '/my-matches', '/profile']);
  });

  it('proScout menu contains no age-groups, users, or observers entry (FR-009)', async () => {
    await setup('proScout');
  ...
  it('a hidden entry is absent from the DOM, not merely styled hidden (FR-002)', async () => {
    await setup('proScout');
    // Only 4 anchors exist at all in the nav — not 7 with 3 hidden via CSS.
